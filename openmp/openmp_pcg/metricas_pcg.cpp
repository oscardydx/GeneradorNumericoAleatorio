...
#include "pcg_random.hpp" // Incluye PCG
...
void stats(const int size, const int reps, double &mean_wtime, double &sigma_wtime, double &mean_ctime, double &sigma_ctime) {
...
void generate_random_numbers(const int NDatos, double &wtime, double &ctime) {
    ...
    // Generar números aleatorios y simular uso
    #pragma omp parallel
    {
        pcg_extras::seed_seq_from<std::random_device> seed_source;
        pcg32 rng(seed_source);
